...
// Injeção segura da chave de API a partir do painel da Vercel
...
export default function App() {
  ...
  // --- CONTROLE DE MONETIZAÇÃO LOCAL ---
  const [usageCount, setUsageCount] = useState(0);
  const [isPremium, setIsPremium] = useState(false);

  // Estados Base de Seleção Cascata
  ...
  // Carregamento inicial do status premium / contagem
  useEffect(() => {
  ...
  // Cascata BNCC Nível -> Ano -> Área -> Disciplina
  useEffect(() => {
  ...
  const generate = async (type) => {
    ...
    // Esquemas rígidos com tipagem explícita para evitar transbordamento de dados
    const planSchema = {
      type: "OBJECT",
      properties: {
        titulo: { type: "STRING" },
      ...
      required: ["titulo", "codigosBNCC", "competenciasEspecificas", "objetivosEspecificos", "materiais", "metodologia", "adaptacao", "avaliacao"]
    };
      ...
      properties: {
        ...
        questoes: {
          ...
          items: {
            ...
            required: ["enunciado", "opcoes", "gabarito"]
          }
        }
      },
      required: ["titulo", "contexto", "questoes"]
    };
    ...
    const systemPrompt = isPlan 
      ? `Você é um Consultor Pedagógico Especialista na BNCC Brasileira.
         ...
         1. Monte um plano de aula completo estruturado estritamente de acordo com o esquema JSON fornecido.
         2. No campo "titulo", coloque apenas o título do plano de aula ou o nome do assunto abordado. Não coloque a metodologia inteira aqui dentro.
         3. Identifique de 2 a 3 códigos alfanuméricos REAIS e exatos da BNCC correspondentes.`
      : `Você é um Professor experiente de ${displaySubject} do ${selectedLevel} (${selectedYear}).
         
         Instruções obrigatórias:
         1. Crie uma atividade avaliativa com exatamente ${meta.numQuestoes} questões seguindo o esquema JSON de forma exata.
         2. No campo "titulo", coloque apenas o título da atividade.
         3. Dificuldade: ${meta.nivelDificuldade}. Formato: ${meta.formatoQuestoes}.`;

    const userQuery = `
      ${contextSource}
      Duração da Aula: ${meta.tempoAula} minutos.
      Código Extra informado manualmente: ${meta.extraSkills || 'Nenhum.'}
    `;
    ...
    try {
      ...
      console.error(e);
      setError(`Falha ao gerar conteúdo: ${e.message}. Tente novamente.`);
    } finally {
      setLoading(false);
    }
  };
